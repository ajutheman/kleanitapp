...
import 'package:kleanitapp/features/address/screens/Appaddress_list_screen.dart';
...
import 'package:kleanitapp/features/order/model/Apporder_calculation.dart';

import '../../../presentation/widgets/Apperror_widget.dart';
import '../../address/bloc/address_state.dart';
import '../../address/model/Appaddress.dart';
...
class AppOrderConfirmationScreen extends StatefulWidget {
  final List<AppCart> carts;

  const AppOrderConfirmationScreen({super.key, required this.carts});

  @override
  State<AppOrderConfirmationScreen> createState() => _AppOrderConfirmationScreenState();
}
...
class _AppOrderConfirmationScreenState extends State<AppOrderConfirmationScreen> {
  ...
  AppTimeSchedule? selectedSchedule;
  AppCustomerAddress? currentAddress;
  List<AppTimeSchedule> timeSchedules = [];
  ...
  String selectedPaymentMethod = AppPaymentMethod.STRIPE;
  ...
  void initState() {
    ...
    isShowSubFeq = widget.carts.length == 1 && widget.carts.first.type == AppSubscriptionType.SUBSCRIPTION;
    if (isShowSubFeq) {
      selectedSubFeq = 1;
    }
  }
  ...
  void _processPayment(BuildContext context, AppOrderCalculation order) async {
      ...
      body: BlocConsumer<OrderBloc, OrderState>(
        listener: (context, state) async {
          if (state is OrderPlaced) {
            ...
            if (selectedPaymentMethod == AppPaymentMethod.STRIPE) {
              try {
                ...
                if (context.mounted) {
                  ...
                  Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => AppHomeMainView()), (predicate) => false);
                }
        ...
        builder: (context, state) {
          if (state is OrderLoading) {
            ...
            return AppMyErrorWidget(onRetry: () => loadOrderCalculations(), message: state.message);
          }
  ...
  Widget buildOrderDetails(BuildContext context, AppOrderCalculation order) {
    return Stack(
      children: [
  ...
  Widget buildAddressDetailsCard(AppCustomerAddress? address, BuildContext context, VoidCallback onAddressChange) {
    return Card(
      elevation: 1,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      child: Padding(
  ...
  Widget buildServiceList(AppOrderCalculation order) {
    return Card(
      elevation: 2,
  ...
  Widget buildPaymentMethodCard(AppOrderCalculation order) {
    return Card(
      elevation: 1,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      child: Padding(
              ...
              children: [
                ...
                RadioListTile<String>(
                  ...
                  onChanged: (String? value) {
                    ...
                    if (value == AppPaymentMethod.WALLET && order.walletAmount < order.requiredAmount) {
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                if (selectedPaymentMethod == AppPaymentMethod.WALLET && order.walletAmount < order.requiredAmount)
                                  Padding(
  ...
  Widget buildPaymentDetailsCard(AppOrderCalculation order) {
    return Card(
      elevation: 2,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      child: Padding(
  ...
  Widget buildPaymentButton(BuildContext context, AppOrderCalculation order) {
    return Container(
      color: Colors.white,
      padding: const EdgeInsets.all(16),
      child: SafeArea(
  ...
  void _selectAddress() async {
    ...
    var address = await Navigator.push(context, MaterialPageRoute(builder: (_) => AppAddressListScreen(isSelectionMode: true)));
    if (address != null) {
      setState(() {
