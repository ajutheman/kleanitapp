...
class AppOrderTypeSelector extends StatefulWidget {
  final Function(String) onTypeSelected;
  final String initialSelection;

  const AppOrderTypeSelector({super.key, required this.onTypeSelected, this.initialSelection = AppSubscriptionType.SINGLE});

  @override
  State<AppOrderTypeSelector> createState() => _AppOrderTypeSelectorState();
}

class _AppOrderTypeSelectorState extends State<AppOrderTypeSelector> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: 2, vsync: this, initialIndex: _selectedType == AppSubscriptionType.SINGLE ? 0 : 1);
  ...
  void _handleTabSelection() {
    if (_tabController.indexIsChanging) {
      setState(() {
        _selectedType = _tabController.index == 0 ? AppSubscriptionType.SINGLE : AppSubscriptionType.SUBSCRIPTION;
      });
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.08), spreadRadius: 0, blurRadius: 15, offset: Offset(0, 5))],
      ),
      child: TabBar(
