...
class AppHomeRepository {
  ...
  Future<AppServiceAvailabilityResponse> setLocation(double latitude, double longitude, String token) async {
    final url = Uri.parse(baseUrl);
    final response = await http.post(
      url,
      headers: {
    ...
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      return AppServiceAvailabilityResponse.fromJson(data);
    }
    if (response.statusCode == 401) {
      throw AppUnauthorizedException("Unauthorized: Token expired or invalid");
    } else if (response.statusCode == 400) {
      throw AppBadRequestException("Bad Request: Invalid location data");
