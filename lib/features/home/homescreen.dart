...
import 'package:kleanit/features/home/faq_section.dart';
import 'package:kleanit/features/home/refer_advertisement_list.dart';
import 'package:kleanit/features/home/repo/WalletScreen.dart';
import 'package:kleanit/features/home/service_type_selector.dart';
import 'package:kleanit/features/home/sub_advertisement_list.dart';
import 'package:kleanit/features/notification/screen/notification_screen.dart';
...
import '../categories/respository/category_repository.dart';
import 'bloc/home_bloc.dart';
import 'category_list.dart';
import 'home_repository.dart';
import 'sub_advertisement_list.dart'; // ✅ ADD THIS LINE
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    context
        .read<CategoriesBloc>()
        .add(LoadCategories(type: _selectedServiceType));
    _getCustomerData();
  }
            ...
            providers: [
              ...
              BlocProvider<AdvertisementBloc>(
                create: (context) => AdvertisementBloc(
                  ...
                  token: token,
                )..add(LoadAdvertisements()),
              ),
              BlocProvider<SubAdvertisementBloc>(
                create: (context) => SubAdvertisementBloc(
                  repository: SubAdvertisementRepository(),
                  token: token,
                )..add(LoadSubAdvertisements()),
              ),
              BlocProvider<ReferalAdvertisementBloc>(
                create: (context) => ReferalAdvertisementBloc(
                  repository: ReferalAdvertisementRepository(),
                  token: token,
                )..add(LoadReferalAdvertisements()),
              ),
            ],
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          getCustomFont("Dubai, UAE", 14, Colors.white, 1,
                            ...
                            children: [
                              BlocBuilder<WalletBloc, WalletState>(
                                ...
                                if (state is WalletLoading) {
                                  ...
                                  return getCustomFont(state.balance.toString(),
                                      15, Colors.white, 1,
                    ...
                    Expanded(
                      child: ListView(
                        ...
                        children: [
                                ...
                                return AdvertisementCarousel(
                                  ...
                                  onBookTap: (advertisement) {
                                    ...
                                    if (categoryBLoc is CategoriesLoaded) {
                                      try {
                                        var cat = categoryBLoc.categories
                                            .firstWhere((e) =>
                                                e.id ==
                                                advertisement.mainCategoryId);
                                        Navigator.pushNamed(
                                          context,
                          ...
                          //   },
                          // ),
                          BlocBuilder<SubAdvertisementBloc,
                              SubAdvertisementState>(
                            builder: (context, state) {
                              if (state is SubAdvertisementLoading) {
                                return ShimmerSubAdvertisementLoader(); // Shimmer effect while loading
                              } else if (state is SubAdvertisementLoaded) {
                                ...
                                final activeAds = state.subAdvertisements
                                    .where((ad) => !ad.isUsed)
                                    .toList();
                                ...
                                return SubAdvertisementList(
                                  ...
                                  // sortedAds,
                                  onBookTap: (advertisement) {
                                    final categoryBlocState =
                                        context.read<CategoriesBloc>().state;
                                    if (categoryBlocState is CategoriesLoaded) {
                                      try {
                                        var cat = categoryBlocState.categories
                                            .firstWhere((e) =>
                                                e.id ==
                                                advertisement.mainCategoryId);
                                        Navigator.pushNamed(
                          ...
                          const SizedBox(height: 10),
                          BlocBuilder<ReferalAdvertisementBloc,
                              ReferalAdvertisementState>(
                            builder: (context, state) {
                              if (state is ReferalAdvertisementLoading) {
                                ...
                                return ReferalAdvertisementList(
                                    referals: state.referalAdvertisements);
