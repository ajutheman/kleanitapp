...
class ReferalAdvertisementRepository {
  ...
  Future<List<ReferalAdvertisement>> fetchReferalAdvertisements(
      String token) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['status'] == true) {
        ...
        return referalData
            .map((json) => ReferalAdvertisement.fromJson(json))
            .toList();
      } else {
        throw Exception(
            "Failed to load referral advertisements: ${data['message']}");
      }
    } else {
      throw Exception(
          "Failed to load referral advertisements: ${response.statusCode}");
    }
  }
}
