...
import 'package:kleanitapp/features/home/repo/Appexceptions.dart' show AppBadRequestException, AppUnauthorizedException;
import 'package:shared_preferences/shared_preferences.dart';

import '../modle/Appwallet_details.dart';
import 'AppWalletTransaction.dart';
...
class AppWalletRepository {
  ...
  Future<double> fetchWalletBalance() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw AppBadRequestException("Invalid wallet balance request.");
      } else {
  ...
  Future<AppWalletDetails> fetchWalletDetails() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString(AppPrefResources.USER_ACCESS_TOCKEN) ?? '';
      ...
      print("🟢 Response Body: ${response.data}");
      if (response.statusCode == 200) {
        return AppWalletDetails.fromJson(response.data);
      } else if (response.statusCode == 401) {
        throw AppUnauthorizedException("Session expired. Please log in again.");
      } else if (response.statusCode == 400) {
        throw AppBadRequestException("Invalid wallet detail request.");
      } else {
  ...
  Future<List<AppWalletTransaction>> fetchReceivedTransactions() async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString(AppPrefResources.USER_ACCESS_TOCKEN) ?? '';
    ...
    if (response.statusCode == 200 && response.data['success']) {
      return (response.data['data'] as List).map((json) => AppWalletTransaction.fromJson(json, isReceived: true)).toList();
    } else if (response.statusCode == 401) {
      throw AppUnauthorizedException("Session expired. Please log in again.");
    } else if (response.statusCode == 400) {
      throw AppBadRequestException("Invalid transaction request.");
    } else {
      throw Exception("Failed to fetch received transactions");
    }
  }

  Future<List<AppWalletTransaction>> fetchSentTransactions() async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString(AppPrefResources.USER_ACCESS_TOCKEN) ?? '';
    ...
    print("🟢 Response Body: ${response.data}");
    if (response.statusCode == 200 && response.data['success']) {
      return (response.data['data'] as List).map((json) => AppWalletTransaction.fromJson(json, isReceived: false)).toList();
    } else {
