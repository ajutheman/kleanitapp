...
class AppWalletScreen extends StatefulWidget {
  const AppWalletScreen({Key? key}) : super(key: key);
...
class _AppWalletScreenState extends State<AppWalletScreen> {
  ...
  AppWalletDetails? walletDetails;
  bool isLoading = true;

  List<AppWalletTransaction> sentTransactions = [];
  List<AppWalletTransaction> receivedTransactions = [];
  ...
  Future<void> _loadWalletDetails() async {
    try {
      final details = await AppWalletRepository().fetchWalletDetails();
      final sent = await AppWalletRepository().fetchSentTransactions();
      final received = await AppWalletRepository().fetchReceivedTransactions();
