...
class _WeeklyScheduleSelectorState extends State<WeeklyScheduleSelector>
  ...
  void initState() {
    ...
    context
        .read<WeeklyScheduleBloc>()
        .add(FetchWeeklySchedule(orderId: widget.orderId));
  }
    ...
    return BlocConsumer<WeeklyScheduleBloc, WeeklyScheduleState>(
      ...
      builder: (context, state) {
        if (state is WeeklyScheduleLoaded) {
          final currentWeek = state.schedule.firstWhere((s) =>
              s.startDate.isBefore(DateTime.now()) &&
              s.endDate.isAfter(DateTime.now()));
          final nextWeek = state.schedule.firstWhere((s) =>
              s.startDate.isBefore(DateTime.now().add(Duration(days: 7))) &&
              s.endDate.isAfter(DateTime.now().add(Duration(days: 7))));
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    "Maximum ${state.schedule.first.weekNumber} days",
                    ...
                    if (isSelected) {
                      ...
                      if (_selectedCount(schedule.days) < schedule.weekNumber) {
                        ...
                        Fluttertoast.showToast(
                            msg:
                                'You can select maximum ${schedule.weekNumber} days');
                      }
                    }
                    setState(() {});
                  },
              ...
              onPressed: () {
                context.read<WeeklyScheduleBloc>().add(
                      UpdateWeeklySchedule(
                          id: schedule.id, days: schedule.days),
                    );
