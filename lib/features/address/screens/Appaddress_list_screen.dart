...
import '../model/Appaddress.dart';
import 'Appaddress_form_screen.dart';

class AppAddressListScreen extends StatelessWidget {
  final bool isSelectionMode;

  const AppAddressListScreen({super.key, this.isSelectionMode = false});
  ...
  Widget _buildAddressCard(BuildContext context, AppCustomerAddress address) {
  ...
  void _navigateToAddressForm(BuildContext context, {AppCustomerAddress? address}) async {
    final result = await Navigator.push(context, MaterialPageRoute(builder: (_) => AppAddressFormScreen(address: address)));
    if (result == true) {
      context.read<AddressBloc>().add(FetchAddresses());
    }
  }

  void _confirmDelete(BuildContext context, AppCustomerAddress address) async {
