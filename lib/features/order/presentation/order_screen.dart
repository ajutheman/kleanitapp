...
import 'package:kleanit/features/address/bloc/address_event.dart';
import 'package:kleanit/features/address/screens/address_list_screen.dart';
import 'package:kleanit/features/cart/model/cart.dart';
import 'package:kleanit/features/home/home_main.dart';
import 'package:kleanit/features/order/bloc/coupon/coupon_event.dart';
import 'package:kleanit/features/order/bloc/order_bloc.dart';
import 'package:kleanit/features/order/bloc/order_event.dart';
import 'package:kleanit/features/order/bloc/order_state.dart';
import 'package:kleanit/features/order/model/order_calculation.dart';
...
class _OrderConfirmationScreenState extends State<OrderConfirmationScreen> {
  ...
  void initState() {
    ...
    isShowSubFeq = widget.carts.length == 1 &&
        widget.carts.first.type == SubscriptionType.SUBSCRIPTION;
    if (isShowSubFeq) {
      selectedSubFeq = 1;
    }
  }
  ...
  loadOrderCalculations() {
    ...
    context.read<OrderBloc>().add(CalculateOrder(
          cartIds: cartIds, coupon: appliedCoupon,
    ...
    setState(() {
      timeSchedules = timeSchedules.where((schedule) {
        final scheduleDateTime =
            DateTime.parse(schedule.scheduleTimeRaw).toLocal();
          ...
          if (state is OrderPlaced) {
            ...
            if (selectedPaymentMethod == PaymentMethod.STRIPE) {
              try {
                await stripe.Stripe.instance.initPaymentSheet(
                  paymentSheetParameters: stripe.SetupPaymentSheetParameters(
                    paymentIntentClientSecret:
                        state.orderResponse.stripePaymentIntent!.clientSecret,
                ...
                if (context.mounted) {
                  ...
                  Navigator.pushAndRemoveUntil(
                      context,
                      MaterialPageRoute(builder: (_) => HomeMainView()),
                      (predicate) => false);
                }
              ...
              if (context.mounted) {
                currentHomeIndexNotifier.value = 1;
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                    content: Text("Order Placed Successfully using Wallet!")));
                Navigator.pushAndRemoveUntil(
                    context,
                    MaterialPageRoute(builder: (_) => HomeMainView()),
                    (predicate) => false);
        ...
        builder: (context, state) {
          if (state is OrderLoading) {
            ...
            return MyErrorWidget(
                onRetry: () => loadOrderCalculations(), message: state.message);
          }
    ...
    return Stack(
      children: [
        ...
        Positioned(
          ...
          left: 0,
          right: 0,
          child: buildPaymentButton(context, order),
        ),
      ],
    );
  }
          ...
          children: [
            ...
            Wrap(
              ...
              children: timeSchedules.map((schedule) {
                final isAvailable =
                    schedule.availableSlots > 0 && schedule.available;
                final isSelected =
                    selectedSchedule?.primaryId == schedule.primaryId;
                  ...
                  onTap: () {
                    if (isAvailable) {
                      ...
                      showSnackBar(context,
                          msg: "No slots available for this time");
                    }
                  },
                      ...
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            Text(
                              isAvailable
                                  ? '${schedule.availableSlots} slot${schedule.availableSlots > 1 ? 's' : ''} available'
                                  : 'No slots',
                              style: TextStyle(
                                fontSize: 8,
                                color:
                                    isAvailable ? Colors.grey[600] : Colors.red,
                              ),
                            ),
                          ],
                        ),
        ...
        child: Column(
          ...
          children: [
                ...
                Expanded(
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: selectedPropertyType == PropertyType.residential
                ...
                Expanded(
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: selectedPropertyType == PropertyType.commercial
                            ? primaryColor.withOpacity(0.1)
                            : Colors.white,
                        border: Border.all(
                          color: selectedPropertyType == PropertyType.commercial
            ...
            if (selectedPropertyType == PropertyType.commercial) ...[
              ...
              buildNumberPickerRow(
                  "Beds", beds, (v) => setState(() => beds = v)),
              buildNumberPickerRow(
                  "Sofa Beds", sofaBeds, (v) => setState(() => sofaBeds = v)),
              ...
              buildNumberPickerRow(
                  "Occupancy", occupancy, (v) => setState(() => occupancy = v)),
              ...
              buildDropdownRow(
                  "Type of Cleaning", cleaningOptions, typeOfCleaning, (val) {
                setState(() => typeOfCleaning = val);
              }),
              // buildTimePickerRow("Check-In Time", checkInTime, (time) => setState(() => checkInTime = time)),
              buildTimePickerRow("Check-Out Time", checkOutTime,
                  (time) => setState(() => checkOutTime = time)),

              // buildTimePickerRow("Check-Out Time", checkOutTime, (time) => setState(() => checkOutTime = time)),
              buildTimePickerRow("Next Guest Check-In", nextGuestCheckInTime,
                  (time) => setState(() => nextGuestCheckInTime = time)),

              buildTextInput("Door Access Code", doorAccessCode,
                  (val) => setState(() => doorAccessCode = val)),
              buildTextInput("WiFi Access Code", wifiAccessCode,
                  (val) => setState(() => wifiAccessCode = val)),
              ...
              buildSwitchRow("Pets Present", petsPresent,
  ...
  Widget buildTimePickerRow(
      String label, String value, Function(String) onPicked) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 6),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
  ...
  Widget buildAddressDetailsCard(CustomerAddress? address, BuildContext context,
      VoidCallback onAddressChange) {
    return Card(
      elevation: 1,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      child: Padding(
        ...
        children: [
          ...
          ClipRRect(
            ...
            child:
                ...
                //   },
                // ),
                (matchingCart.thirdCategory.image != null &&
                        matchingCart.thirdCategory.image.isNotEmpty)
                    ? Image.network(
                        matchingCart.thirdCategory.image!,
                        width: 60,
                        height: 60,
                  ...
                  onChanged: (String? value) {
                    ...
                    if (value == PaymentMethod.WALLET &&
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: const Text(
                              "You don't have enough wallet coins to place this order."),
                                  ...
                                  Padding(
                                    ...
                                    child: Row(
                                      children: [
                                        ...
                                        const SizedBox(width: 2),
                                        Text(
                                          "Not enough coin wallet balance",
                                          style: TextStyle(
                                            fontSize: 10,
              ...
              children: [
                    ...
                    ElevatedButton(
                      onPressed: () {
                        if (_couponController.text.trim().isNotEmpty) {
                          context.read<CouponBloc>().add(ValidateCoupon(
                ...
                if (state is! CouponInitial) ...[
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      color: (state is CouponValidationFailure)
                          ? Colors.red[700]
                          : Colors.green[700],
                      fontWeight: FontWeight.w500,
                    ),
        ...
        child: Column(
          ...
          children: [
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          const SizedBox(width: 4),
                          Text(
                            "AED ${order.taxAmount.toStringAsFixed(2)}",
                            style: const TextStyle(fontSize: 15),
                          ),
                        ],
                      ),
                  ...
                  if (order.couponDiscount != 0) ...[
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          "AED -${order.couponDiscount.toStringAsFixed(2)}",
                          style: const TextStyle(
            ...
            if (widget.carts.isNotEmpty &&
                widget.carts.first.thirdCategory.walletCoins.isNotEmpty) ...[
              const SizedBox(height: 16),
              Container(
                padding:
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 2),
                          Text(
                            "You'll earn ${order.walletCoin} wallet coins with this purchase!",
                            style: TextStyle(
                              color: Colors.amber[800],
                              fontSize: 13,
                            ),
        ...
        child: GestureDetector(
          onTap: () {
            if (!formIsValid) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
                    "❗ Please fill all required commercial property details.",
          ...
          child: AbsorbPointer(
            ...
            child: ElevatedButton(
              onPressed:
                  formIsValid ? () => _processPayment(context, order) : null,
              style: ElevatedButton.styleFrom(
                backgroundColor: primaryColor,
                foregroundColor: Colors.white,
                padding: const EdgeInsets.symmetric(vertical: 16),
    ...
    context.read<AddressBloc>().stream.listen((state) {
      if (state is AddressLoaded) {
        ...
        if (addressList.isNotEmpty) {
          currentAddress = addressList.firstWhere(
            (a) => a.isDefault == 1,
  ...
  void _selectAddress() async {
    ...
    var address = await Navigator.push(
        context,
        MaterialPageRoute(
            builder: (_) => AddressListScreen(isSelectionMode: true)));
    if (address != null) {
      setState(() {
  ...
  Widget buildDropdownRow(String label, List<String> options, String selected,
