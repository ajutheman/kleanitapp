...
class AppCartRepository {
  ...
  // ✅ Fetch Cart List
  Future<List<AppCart>> fetchCartList() async {
    try {
      ...
      if (response.statusCode == 200) {
        return AppCart.listFromJson(response.data['data']);
      } else {
  ...
  Future<int> addToCart({required int thirdCategoryId, required int employeeCount, required String type, required int subscriptionFrequency}) async {
    try {
      ...
      final payload = {
        ...
        "type": type,
        if (type == AppSubscriptionType.SUBSCRIPTION) "subscription_frequency": subscriptionFrequency,
      };
